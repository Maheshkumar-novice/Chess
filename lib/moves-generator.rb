...
class MovesGenerator
  def create_moves(cell, board, moves = [])
    move_methods(board[cell].piece.name).each { |method| moves += send(method, cell, board) }
    moves
  end
  ...
  def horizontal_moves(cell, board)
    generate_moves_from_directions(%i[row_right row_left], cell, board)
  end

  def vertical_moves(cell, board)
    generate_moves_from_directions(%i[column_above column_below], cell, board)
  end

  def knight_moves(cell, board, moves = [])
    ...
    first_steps.each do |first_step|
      ...
      moves += generate_moves_from_possible_steps(second_steps_for_this_first_step, current_cell, board)
    end
    moves
  end
  ...
  def king_moves(cell, board)
    steps_possible = %i[row_right
                        ...
                        bottom_right_diagonal
                        bottom_left_diagonal]
    generate_moves_from_possible_steps(steps_possible, cell, board)
  end

  def pawn_moves(cell, board)
    return white_pawn_moves(cell, board) if board[cell].piece.color == 'white'

    black_pawn_moves(cell, board) if board[cell].piece.color == 'black'
  end

  def white_pawn_moves(cell, board)
    moves = generate_moves_from_possible_steps(%i[column_above top_left_diagonal top_right_diagonal], cell, board)
    moves << add_double_step(cell, board, :column_above) if cell.match?(/^[a-h]2$/)
    moves
  end

  def black_pawn_moves(cell, board)
    moves = generate_moves_from_possible_steps(%i[column_below bottom_left_diagonal bottom_right_diagonal], cell, board)
  ...
  def generate_moves_from_directions(directions, cell, board, moves = [])
    directions.each do |direction|
      moves += generate_moves_recursively(board[cell].send(direction), board, direction) if board[cell].send(direction)
    end
    moves
  end

  def generate_moves_from_possible_steps(steps_possible, cell, board, moves = [])
  ...
  def generate_moves_recursively(cell, board, direction, moves = [])
